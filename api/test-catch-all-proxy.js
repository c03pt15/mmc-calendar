export default async function handler(req, res) {
  console.log('=== TESTING CATCH-ALL PROXY ===');
  ...
  try {
    ...
    res.status(200).json({
      message: 'Catch-all proxy test completed',
    ...
    console.error('Catch-all proxy test error:', error);
    res.status(500).json({
      error: 'Catch-all proxy test failed',
